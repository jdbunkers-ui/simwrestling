(async function () {
  ...
  try {
    ...
    const topTraits = [...a].sort((x,y) => Number(y.rating_range_maximum)-Number(x.rating_range_maximum)).slice(0,2).map((x) => x.category_name);
    ...
    root.innerHTML = `
      <section class="profile-hero">
        <div class="athlete-id"><div class="monogram" aria-hidden="true">${e(initials)}</div><div>
          ...
          <div class="style-tags"><span>${e(p.stance_display)}</span>${topTraits.map((trait) => `<span>${e(trait)}</span>`).join('')}</div>
        </div></div>
      ...
      <div class="tab-panel" id="public-panel">
        <section class="section-block"><div class="section-heading"><div><p class="eyebrow">Scouting profile</p><h2>Attribute Summary</h2></div><p class="section-note">Descriptive bands are public. Exact engine attributes remain hidden.</p></div>
          <div class="attribute-grid">${a.map((item) => `<article class="attribute-card"><div><h3>${e(item.category_name)}</h3><p>${e(item.category_description)}</p></div><span class="rating">${e(item.rating_label)}<small>${e(item.rating_range_display)}</small></span></article>`).join('')}</div>
        </section>
        <section class="two-column section-block"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">Career totals</p><h2>Media Statistics</h2></div><span class="record-pill">${m.match_qty || 0} matches</span></div><div class="media-stats">
          ${stat('Takedowns',m.takedown_qty || 0,`${pct(m.takedown_success_rate)} conversion`)}${stat('Escapes',m.escape_qty || 0,'successful escapes')}${stat('Reversals',m.reversal_qty || 0,'successful reversals')}${stat('Back points',m.back_point_qty || 0,'near-fall points')}${stat('Riding time',m.riding_time_display || '0:00',`${num(m.average_riding_time_seconds,0)} sec. per match`)}${stat('Team points',num(m.team_points_earned,1),`${pct(bonusRate)} bonus wins`)}</div></article>
          <article class="panel"><div class="panel-heading"><div><p class="eyebrow">Recent results</p><h2>Latest Matches</h2></div></div>${history.length ? `<div class="history-list">${history.slice(0,5).map((match) => `<div class="history-row"><span>${match.match_date ? new Date(match.match_date).toLocaleDateString(undefined,{month:'short',day:'numeric'}) : '—'}</span><strong>${e(match.opponent_name)}<span>${e(match.match_context)}</span></strong><b class="${match.result_outcome === 'WIN' ? 'result-win' : 'result-loss'}">${e(match.result_outcome === 'WIN' ? 'W' : 'L')} ${e(match.score_display)}</b><span>${e(String(match.result_type).replaceAll('_',' '))}</span></div>`).join('')}</div>` : '<p class="coach-empty">No completed matches yet.</p>'}</article>
        </section>
      </div>
