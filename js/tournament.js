(async function () {
  ...
  async function renderSelector() {
    ...
    root.innerHTML = `<section class="page-heading"><p class="eyebrow">State championship test</p><h1>Run Tournament</h1><p>Select a state and weight class. All 16 wrestlers enter a complete double-elimination bracket with no byes.</p></section>
      <section class="tournament-selector panel"><div><label class="filter-control">State<select id="tournament-state">${states.map((state) => `<option value="${e(state)}" ${state === initialState ? 'selected' : ''}>${e(state)}</option>`).join('')}</select></label><label class="filter-control">Weight class<select id="tournament-weight"></select></label></div><div class="tournament-format"><span>16</span><p><strong>Wrestlers</strong><small>28 bouts · places 1–4</small></p></div><button class="primary-button launch-button" id="run-tournament" type="button">Build the Bracket</button></section><p class="section-note event-message" id="tournament-status"></p>`;
    ...
    button.addEventListener('click', async () => {
      button.disabled = true; button.textContent = 'Wrestling 28 Bouts…'; status.textContent = 'The engine is resolving the complete bracket. This can take a moment.';
      try {
  ...
  async function renderTournament() {
    ...
    root.innerHTML = `<section class="tournament-header"><div><p class="eyebrow">Double elimination · Engine v0.1.3</p><h1>${e(tournament.tournament_name)}</h1><p>${tournament.entrant_qty} wrestlers · ${tournament.bout_qty} matches · complete bracket with no byes</p></div><div class="tournament-live"><span id="tournament-counter">Opening bracket</span><strong id="tournament-round">Seeds locked</strong></div></section>
      ...
      <section class="bracket-section"><div class="bracket-section-heading"><span>Championship bracket</span><small>All 16 seeds begin in Round 1</small></div><div class="bracket-grid championship-grid">${bracketColumn('Round 1',rounds(['CHAMP_R1']))}${bracketColumn('Quarterfinals',rounds(['CHAMP_QF']))}${bracketColumn('Semifinals',rounds(['CHAMP_SF']))}${bracketColumn('Championship',rounds(['CHAMP_FINAL']))}</div></section>
      <section class="bracket-section consolation-section"><div class="bracket-section-heading"><span>Wrestlebacks</span><small>One more loss ends the tournament</small></div><div class="bracket-grid consolation-grid">${bracketColumn('Round 1',rounds(['CONS_R1']))}${bracketColumn('Round 2',rounds(['CONS_R2']))}${bracketColumn('Quarterfinals',rounds(['CONS_QF']))}${bracketColumn('Semifinals',rounds(['CONS_SF']))}${bracketColumn('Third Place',rounds(['THIRD_PLACE']))}</div></section>
